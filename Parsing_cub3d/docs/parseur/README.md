...
- `parse_texture_path` stocke `NO/SO/WE/EA` (trim des espaces de fin, refus d’espace interne, chemin non vide unique, exige `.xpm` lisible).
- `parse_color_value` convertit `F` et `C` en `t_color` en supposant trois segments séparés par des virgules : `parse_component` accepte les espaces autour de chaque nombre et valide 0..255, mais aucune vérification n’assure qu’il y a exactement trois valeurs (au-delà de trois ignorées).
